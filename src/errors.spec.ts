...
test('SimulationMismatch error', () => {
  const action = { type: 'fn/callback', payload: { a: 1 }, meta: { id: 3 } }
  ...
  t.equal(err.message, `Recorded data for 'some id' doesn't match with simulation. Expecting action type 'fn/invoke' but received: { type: 'fn/callback', payload: { a: 1 }, meta: { id: 3 } }`)
})
...
test('SimulationMismatch error with expected action as SpecAction', () => {
  ...
  t.equal(err.receivedAction, action)
  t.equal(err.message, `Recorded data for 'some id' doesn't match with simulation. Expecting action type { type: 'fn/invoke', payload: [0, 'a'] } but received: { type: 'fn/callback', payload: { a: 1 }, meta: { id: 3 } }`)
})
